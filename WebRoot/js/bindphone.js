...
var code ;
function submit(){
	var inputCode = $("#user-new-code").val();
	var inputPhone = $("#user-new-phone").val();
...
function sendMobileCode(){
	if(isEmply("user-new-phone")){
		alert('手机不能为空')
		return;
	}
	$("#sendMobileCode").attr("onclick"," ");
	timeout = false;
	var email = $('#user-new-phone').val();
	var data = {'user.phone':email};
	code = ajaxSend("safeSetUp_sendPhoneCode",data);
	console.log(code);
	time();
}
//定时器，调用自身，递归
