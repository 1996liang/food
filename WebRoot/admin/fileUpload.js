$(function(){
	...
	// 文件上传过程中创建进度条实时显示。
	uploader.on( 'uploadProgress1', function( file, percentage1 ) {
		var $li = $( '#'+file.id ),
		...
		$percent.css( 'width', percentage1 * 100 + '%' );
	});
...
(function( $ ){
    ...
    $(function() {
        var $wrap = $('.uploader-list-container1'),
            ...
            // 状态栏，包括进度和控制按钮
            $statusBar1 = $wrap.find( '.statusBar1' ),

            // 文件总体选择信息。
            $info1 = $statusBar1.find( '.info1' ),

            // 上传按钮
            $upload = $wrap.find( '.uploadBtn1' ),

            // 没选择文件之前的内容。
            $placeHolder1 = $wrap.find( '.placeholder1' ),

            $progress1 = $statusBar1.find( '.progress1' ).hide(),
        ...
        function addFile( file ) {
            ...
            $li.on( 'mouseenter', function() {
                $btns1.stop().animate({height: 30});
            });

            $li.on( 'mouseleave', function() {
                $btns1.stop().animate({height: 0});
            });

            $btns1.on( 'click', 'span', function() {
        ...
        function setState( val ) {
            ...
            switch ( state ) {
                ...
                case 'ready':
                    ...
                    $( '#filePicker21' ).removeClass( 'element-invisible');
                    $queue.show();
                ...
                case 'uploading':
                    $( '#filePicker21' ).addClass( 'element-invisible' );
                    $progress1.show();
                    $upload.text( '暂停上传' );
                    break;
        ...
        uploader.onUploadProgress1 = function( file, percentage1 ) {
            var $li = $('#'+file.id),
                $percent = $li.find('.progress1 span');

            $percent.css( 'width', percentage1 * 100 + '%' );
            percentages[ file.id ][ 1 ] = percentage1;
            updateTotalProgress1();
        };
